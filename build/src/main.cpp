...
void process_packet(int client_id, std::vector<std::tuple<std::string, std::string, std::string>> records, std::vector<char> packet)
{
    ...
    bindserv::sendback(client_id, &bytes[0], bytes.size());
}
